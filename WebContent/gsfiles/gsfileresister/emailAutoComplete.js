...
function EmailAutoComplete(options) {
    ...
    this.config = {
        targetCls: '.emailAutoComplete',       // 目标input元素
        parentCls: '.emailAutoParent',       // 当前input元素的父级类
        hiddenCls: '.hiddenCls',       // 当前input隐藏域 
        searchForm: '#DialogForm', //form表单
...
EmailAutoComplete.prototype = {
        ...
        $(_config.targetCls).each(function (index, item) {
            ...
            $(item).keyup(function (e) {
                ...
                if (targetVal == '') {
                    ...
                    _cache.oldIndex = -1;
                    $(".auto-tip", parentNode) && !$(".auto-tip", parentNode).hasClass('hidden') && $(".auto-tip", parentNode).addClass('hidden').hide();
                    ...
                    $(".auto-tip", parentNode) && $(".auto-tip", parentNode).hasClass('hidden') && $(".auto-tip", parentNode)
            ...
            if (!$(target).hasClass(tagCls)) {
                $('.auto-tip') && $('.auto-tip').each(function (index, item) {
                    if (!$(item).hasClass('hidden')) {
                        $(item).addClass('hidden').hide("normal", function () {
    ...
    _renderHTML: function (cfg) {
        ...
        if ($('.auto-tip', cfg.parentNode).hasClass('hidden')) {
            $('.auto-tip', cfg.parentNode).removeClass('hidden').show();
        }
        if (curIndex > -1) {
            // 键盘上下操作
            ...
            if (_cache.onlyFlag) {
                ...
                $('.auto-tip', cfg.parentNode).css({
                    ...
                    'border': '1px solid #ccc', 'z-index': 999,'list-style':'none'
                });
            }
    ...
    _accurateMate: function (cfg) {
        ...
        if (/@/.test(curVal)) {
            ...
            if (newArrs.length > 0) {
                ...
                $(".auto-tip", cfg.parentNode) && !$(".auto-tip", cfg.parentNode).hasClass('hidden') &&
				$(".auto-tip", cfg.parentNode).addClass('hidden').hide("normal", function () {
				    _config.callback && $.isFunction(_config.callback) && _config.callback();
				});
                $('.auto-tip', cfg.parentNode).html('');
            }
        ...
        if ($('.auto-tip' + ' li', parentNode) && $('.auto-tip' + ' li').length > 0) {
            ...
            if (keyCode == 38) {
                ...
                if (_config.isSelectHide) {
                    !$(".auto-tip", parentNode).hasClass('hidden') && $(".auto-tip", parentNode).addClass('hidden').hide("normal", function () {
                        _config.callback && $.isFunction(_config.callback) && _config.callback();
                    });
                }
                //_config.callback && $.isFunction(_config.callback) && _config.callback();

...
<body>
	<form class="form-inline" id="signupForm"
		action="${pageContext.request.contextPath}/goodsUpdate.action"
		...
		<input type="hidden" name="goodsId" value="${good.goodsId}"/> 
			<div class="form-group">
				<label>*商品名称:</label> <input type="text" value="${good.goodsName}" class="form-control"
			...
			<div class="form-group">
				<label>*商品类型:</label> <select class="form-control"
					...
					<c:forEach var="goodsType" items="${goodsTypeList}">
						<option value="${goodsType.goodsTypeId}" <c:if test="${goodsType.goodsTypeId==good.goodsTypeId}">selected="selected"</c:if> >
			...
			<div class="form-group">
				<label>*供应商:</label> <select class="form-control"
					...
					<c:forEach var="supplier" items="${supplierList}">
						<option value="${supplier.supplierId}"  <c:if test="${supplier.supplierId==good.supplierId}">selected="selected"</c:if> >
			...
			<div class="form-group">
				<label>*标题:</label> <textarea type="text" name="title" id="title"
					class="form-control"  style="width: 600px"    rows="3" placeholder="前台商品标题.." >${good.title}</textarea>
			</div>
			<br>
			
			<div class="form-group">
				<label>*图片:</label> <input type="File" id="goodsPhotoFile"
					accept=".jpeg,.jpg,.png" id="goodsPhotoFile" name="goodsPhotoFile" />

			</div>
			原图片:<img alt="404" src="/goods/${good.goodsPhoto}" width="100px" height="100px"> <br>
			<br>
			<div class="form-group">
				<label>*价格:</label> <input type="text" name="price" id="price"
					class="form-control" style="width: 80px" onblur="check(this)"
					onkeyup="this.value=this.value.replace(/[^0-9.]/g,'')"
					placeholder="售价" value="${good.price}" /> <label>￥</label>
			</div>
			...
			<div class="form-group">
				<label>建议采购价:</label> <input type="text" name="beforeprice"
					...
					onkeyup="this.value=this.value.replace(/[^0-9.]/g,'')"
					class="form-control" style="width: 80px" value="${good.beforeprice}"/> <label>￥</label>
			</div>
			&nbsp;&nbsp;
			<div class="form-group">
				<label>*单位:</label> <input type="text" name="unit" id="unit"
					class="form-control" placeholder="单位" value="${good.unit}" style="width: 80px" />
			</div>
			<br>
			<div class="form-group">
				<label>最小购买数:</label> <input type="text" name="leastNum" 
					placeholder="输入整数" onblur="check(this)"
					onkeyup="this.value=this.value.replace(/[^0-9.]/g,'')" value="${good.leastNum}"
					class="form-control" style="width: 80px" /> 
			</div>
			&nbsp; &nbsp;
			<div class="form-group">
				<label>最大购买数:</label> <input type="text" name="thirdNum"
					placeholder="输入整数" onblur="check(this)" value="${good.thirdNum}"
					onkeyup="this.value=this.value.replace(/[^0-9.]/g,'')"
					class="form-control" style="width: 80px" />  
			</div>
			...
			<div class="form-group">
				<label>*商品单位大小:</label> <input type="text" name="goodsCapacity"
					...
					class="form-control" value="${good.goodsCapacity}" placeholder="输入单位大小" style="width: 200px" />
			</div>
			<br>
			...
			<div class="form-group">
				...
				<textarea type="text" name="introduce" style="width: 600px" rows="3"
					class="form-control" > ${good.introduce} </textarea>
				<br>
			</div>
		</div>
		<br>
